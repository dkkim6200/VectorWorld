...
public class Display extends Canvas implements Runnable {
	...
	private Vector3[] utahTeapotData;
	...
	public Display() {
		...
		for (int i = 0; i < WIDTH; i++) {
			for (int j = 0; j < HEIGHT; j++) {
				pixels[i][j] = COLOR_BLACK;
			}
		}
		
		scale = 50;
		
		utahTeapotData = new Vector3[data.length];
	...
	public void update() {
		...
		for (int i = 0; i < utahTeapotData.length - 1; i++) {
			utahTeapotData[i].rotate(new Vector3(1, 0, 0), 45.0 / FPS);
		}
	}
	
	public void drawVertex(Vector3 offset, Vector3[] vertex, int[][] patch) {
		for (int i = 0; i < patch.length; i++) {
			for (int j = 0; j < patch[i].length - 1; j++) {
				Vector3 start = vertex[patch[i][j] - 1];
				Vector3 end = vertex[patch[i][j+1] - 1];
